...
class HiSparseUnifiedRadixCache(UnifiedRadixCache):
    ...
    def init_hisparse_radix_cache(
        ...
        # layer-first. For direct I/O, use the direct page-first layout so the
        # page-granular indexer sidecar can follow the KV pool's fast path.
        indexer_layout = (
            "page_first_direct"
    ...
    def backup_from_device_all_layer(
        ...
        if io_backend == "direct":
            ...
            if total_len % self.page_size != 0:
                raise ValueError(
                    "HiSparse unified radix direct I/O expects page-aligned backup "
    ...
    def init_load_back(
        ...
        self.ongoing_load_back[ack_id] = _OngoingLoadBack(
            best_match_node,
            self.inc_lock_ref(best_match_node).to_dec_params(),
            self.inc_host_lock_ref(best_match_node).to_dec_params(),
        )
